...
const POTION_RECIPES: Dictionary = {
	...
	"Mead": {
		...
		"description": "A simple Viking drink. Later this can buff stamina."
	},
	"Rune Tonic": {
		"recipe": {"Rune Dust": 2, "Bone Charm": 1, "Crystal Vial": 1},
		"description": "A magical potion for future spell buffs."
	}
}
...
func _build_ui() -> void:
	...
	header_copy.text = "Inventory, crafting, and settlement building"
	...
	footer_label.text = "I = inventory   C = craft tab   B = build tab   Pick up drops from enemies to gather more materials."
...
func _refresh_inventory(counts: Dictionary) -> void:
	...
	intro.text = "Everything your character is carrying right now. Consumables can be used directly here."
	...
	for key in keys:
		...
		count_label.text = "x%d" % int(counts[key])
...
func _refresh_building(counts: Dictionary) -> void:
	...
	var intro: Label = Label.new()
	intro.text = "Spend materials to place structures on the map near your character."
	intro.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	intro.add_theme_color_override("font_color", Color(0.73, 0.78, 0.86))
...
func _is_usable_item(item_name: String) -> bool:
	return item_name == "Health Potion" or item_name == "Greater Health Potion"
...
func _item_description(item_name: String) -> String:
	match item_name:
		...
		"Health Potion": return "Restores 35 HP when used."
		"Greater Health Potion": return "Restores 75 HP when used."
		"Mead": return "A hearty drink for future buffs."
		"Rune Tonic": return "A magical tonic for future spell effects."
		...
		"Bone Charm": return "A ritual item with mystical properties."
